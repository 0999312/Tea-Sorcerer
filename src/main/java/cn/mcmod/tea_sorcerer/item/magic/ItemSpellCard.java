...
public class ItemSpellCard extends ItemMagicBase {
	...
	public boolean UsingItemByRightClick(World worldIn, PlayerEntity playerIn, Hand handIn) {
		...
		for(int i = 0;i<75;i++) {
			...
			danmaku.shootFromRotation(playerIn, playerIn.xRot , playerIn.yRot + (float) MathUtil.sinValueIn(360F, i), 0, 2F, 5F);
...
//        playerIn.playSound(, 1.0f, 0.8f);
        worldIn.playSound(null, playerIn.getX(), playerIn.getY(), playerIn.getZ(), SoundRegister.SPELLCARD.get(), playerIn.getSoundSource(), 1.0f, 1.0f);
		return true;
	}
