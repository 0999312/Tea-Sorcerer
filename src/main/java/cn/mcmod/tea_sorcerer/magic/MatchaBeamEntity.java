...
public class MatchaBeamEntity extends AbstractFireballEntity {
    public boolean isCanBreak = false;
    public float explosionPower = 0.1F;

    public MatchaBeamEntity(EntityType<? extends MatchaBeamEntity> p_i50160_1_, World p_i50160_2_) {
        super(p_i50160_1_, p_i50160_2_);
    }

    public MatchaBeamEntity(World worldIn, LivingEntity shooter, double accelX, double accelY, double accelZ) {
        super(EntityRegistry.MATCHA_BEAM, shooter, accelX, accelY, accelZ, worldIn);
    }

    public MatchaBeamEntity(World worldIn, double x, double y, double z, double accelX, double accelY, double accelZ) {
        super(EntityRegistry.MATCHA_BEAM, x, y, z, accelX, accelY, accelZ, worldIn);
    }
    ...
    protected void onImpact(RayTraceResult result) {
        if (!this.world.isRemote) {
            if (result.getType() == RayTraceResult.Type.ENTITY) {
                ...
                boolean flag = entity.attackEntityFrom(DamageSource.causeMobDamage(this.shootingEntity).setProjectile(), 8.0F);
                if (flag) {
